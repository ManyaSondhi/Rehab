...
scoreDisplay.style.textShadow = '2px 2px 4px #000, -2px -2px 4px #000, 2px -2px 4px #000, -2px 2px 4px #000';
...
livesDisplay.style.textShadow = '2px 2px 4px #000, -2px -2px 4px #000, 2px -2px 4px #000, -2px 2px 4px #000';
...
navigator.mediaDevices.getUserMedia({ video: true })
  ...
  .catch(error => {
    ...
    alert("Camera access is required to play this game. Please allow camera access and reload the page.");
  });
...
function spawnFruitOrBomb() {
  ...
  if (isBomb) {
    ...
    img.onerror = () => console.error(`Failed to load bomb image from ${img.src}`);
    ...
    img.onerror = () => console.error(`Failed to load ${type} image from ${img.src}`);
...
function showCameraMessage() {
  ...
  ctx.fillStyle = "#fff";
  ctx.font = "24px Arial";
  ctx.fillText("Please allow camera access to play", canvas.width / 2 - 200, canvas.height / 2);
}
